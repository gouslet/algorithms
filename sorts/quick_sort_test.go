...
func TestQuickSort(t *testing.T) {
	...
	for _, test := range tests {
		...
		if QuickSort(Slice[string](items)); !reflect.DeepEqual(items, res) {
			t.Fatalf("got %v,want %v\n", items, res)
		}
	}
}

func ExampleQuick3WaySort() {
	_, crtfile, _, ok := runtime.Caller(0)
	if !ok {
		panic(errors.New("Can not get current file info"))
	}
...
func TestQuick3WaySort(t *testing.T) {
	...
	for _, test := range tests {
		...
		if Quick3Way(Slice[string](items)); !reflect.DeepEqual(items, res) {
			t.Fatalf("got %v,want %v\n", items, res)
		}
	}
}
